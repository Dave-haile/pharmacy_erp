...
const quickLinks = [
  ...
  { name: "Sales", path: "/sales", icon: <Bell className="w-3 h-3" /> },
  {
    name: "Audit Logs",
    path: "/audit-logs",
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setSearchGroups(
          response.results.filter((group) => group.items.length > 0),
        );
      } catch (error) {
        if (!isActive) {
          return;
      ...
      <div className="flex-1 max-w-xl relative" ref={searchRef}>
        <div className="relative group">
          ...
          <input
            ...
            type="text"
            value={searchQuery}
            placeholder="Search inventory, production, sales, and audit data..."
              ...
              <div className="max-h-[26rem] overflow-y-auto">
                {isSearching && (
                  <div className="px-3 py-4 text-[11px] font-bold text-slate-500 dark:text-slate-400">
                    Searching across inventory, production, and audit records...
                  </div>
                )}
                ...
                {!isSearching && !searchError && searchGroups.length === 0 && (
                  <div className="px-3 py-4 space-y-3">
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mt-1">
                        Try a medicine name, batch number, stock entry
                        reference, or audit action.
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-1">
                      {quickLinks.slice(0, 4).map((link) => (
                        <button
                          key={link.path}
                          onClick={() => handleNavigate(link.path, searchQuery)}
                          className="flex items-center space-x-2.5 px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors text-left border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
                        >
                          <div className="p-1.5 bg-slate-50 dark:bg-slate-800 rounded-lg text-emerald-500">
                            {link.icon}
