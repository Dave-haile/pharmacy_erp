...
import { Department, DepartmentFilters } from "../../types/hr";
...
const DepartmentRegistry: React.FC = () => {
  ...
  const [debouncedFilters, setDebouncedFilters] =
    useState<DepartmentFilters>(filters);
  ...
  const { data, isLoading, isFetching, error, refetch } = useQuery({
    queryKey: ["departments", currentPage, pageSize, debouncedFilters],
    queryFn: () => fetchDepartments(currentPage, pageSize, debouncedFilters),
    staleTime: 60 * 1000,
  ...
  const items = useMemo<Department[]>(
    () => data?.results || [],
    [data?.results],
  );
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-700 dark:text-slate-200">
          ...
          <span>{item.manager_name || "Not assigned"}</span>
        </div>
      ),
    },
    {
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="relative sm:col-span-2 lg:col-span-3">
          ...
          <TextInput
            ...
            placeholder="Search department name, manager, or description..."
            size="sm"
            hasIcon
          />
        </div>
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {totalCount} department{totalCount === 1 ? "" : "s"} available for employee assignment.
          </p>
        </div>
      ...
      <DataTable
        ...
        filters={filtersUI}
        footer={footer}
        onRowClick={(item) => navigate(`/hr/departments/${item.id}`)}
