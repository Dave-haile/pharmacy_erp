...
import { fetchGrns } from "../../services/grn";
import type { GrnSummary } from "../../types/types";
import { DocumentHeader, DocumentPage } from "../../components/common/DocumentUI";

const GrnRegistry: React.FC = () => {
  ...
  const { data, isLoading, isFetching, error, refetch } = useQuery({
    queryKey: ["grn", currentPage, pageSize],
    queryFn: () => fetchGrns(currentPage, pageSize),
    staleTime: 30 * 1000,
  ...
  const columns: Column<GrnSummary>[] = [
    {
      header: "GRN",
      render: (row) => (
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col">
          ...
          <span className="text-[9px] font-mono text-slate-500">{row.invoice_number}</span>
        </div>
      ),
    },
    {
  ...
  return (
    <DocumentPage>
      <DocumentHeader title="Goods Receiving Notes (GRN)" subtitle="Receiving vouchers created from stock entries." />
      <DataTable
        columns={columns}
        data={rows}
        isLoading={isLoading}
        ...
        onRefresh={() => refetch()}
        onRowClick={(row) => navigate(`/inventory/grn/${row.good_reciving_note_id}`)}
        footer={
