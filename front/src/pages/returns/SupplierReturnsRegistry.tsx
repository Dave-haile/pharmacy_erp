...
import { fetchSupplierReturns } from "../../services/supplierReturns";
import type { SupplierReturnSummary } from "../../types/types";
import {
  DocumentHeader,
  DocumentPage,
...
const SupplierReturnsRegistry: React.FC = () => {
  ...
  const { data, isLoading, isFetching, error, refetch } = useQuery({
    queryKey: ["supplier-returns", currentPage, pageSize],
    queryFn: () => fetchSupplierReturns(currentPage, pageSize),
    staleTime: 30 * 1000,
  ...
  useEffect(() => {
    if (!error) return;
    showError("Failed to load supplier returns.");
  }, [error, showError]);

  const rows = useMemo(() => data?.results || [], [data?.results]);

  const columns: Column<SupplierReturnSummary>[] = [
    {
      header: "Posting No",
      render: (row) => (
      ...
      render: (row) => (
        <div className="flex flex-col">
          ...
          <span className="text-[9px] font-mono text-slate-500">
            {row.reference_document || "No reference"}
          </span>
        </div>
      ),
    },
  ...
  return (
    <DocumentPage>
      <DocumentHeader
        ...
        subtitle="Return stock to supplier (decreases inventory)."
        actions={
          <button
            type="button"
      ...
      <DataTable
        ...
        onRefresh={() => refetch()}
        onRowClick={(row) =>
          navigate(`/inventory/supplier-returns/${row.id}`, {
            state: { returnId: row.id },
          })
        }
        footer={
...
export default SupplierReturnsRegistry;
