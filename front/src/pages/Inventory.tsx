...
const UnitConverterModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-4 md:p-6 animate-in zoom-in duration-200">
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-base font-bold text-slate-800">Unit Converter</h3>
          </div>
        ...
        <div className="space-y-4">
          <div className="space-y-3">
            <div className="p-3 md:p-4 bg-slate-50 border border-slate-100 rounded-xl">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Source Value</label>
              <div className="flex items-center space-x-3">
                <input
                  type="number"
                  value={value}
                  onChange={(e) => setValue(Number(e.target.value))}
                  className="flex-1 bg-transparent text-base md:text-lg font-bold text-slate-800 outline-none w-full"
                />
                <select
                  value={fromUnit}
                  onChange={(e) => setFromUnit(e.target.value)}
                  className="bg-white border border-slate-200 rounded-lg px-2 py-1 text-[10px] font-bold text-slate-600 shadow-sm"
                >
            ...
            <div className="flex justify-center relative z-10">
              <button onClick={swapUnits} className="bg-white border border-slate-200 p-2 rounded-full shadow-lg hover:shadow-xl hover:-rotate-180 transition-all duration-500 text-emerald-600">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>
              </button>
            </div>
            <div className="p-3 md:p-4 bg-emerald-50 border border-emerald-100 rounded-xl relative -mt-4 pt-8">
              <label className="text-[9px] font-black text-emerald-600/60 uppercase tracking-widest mb-1.5 block">Resulting Quantity</label>
              <div className="flex items-center justify-between">
                <div className="text-lg md:text-xl font-black text-emerald-800 break-all pr-3">
                  {convertedValue.toLocaleString(undefined, { maximumFractionDigits: 6 })}
                </div>
                <select
                  value={toUnit}
                  onChange={(e) => setToUnit(e.target.value)}
                  className="bg-white border border-emerald-200 rounded-lg px-2 py-1 text-[10px] font-bold text-emerald-700 shadow-sm"
                >
                  {units.map(u => <option key={u} value={u}>{u}</option>)}
                </select>
              </div>
          ...
          <button onClick={copyToClipboard} className="w-full py-3 bg-slate-900 text-white text-sm rounded-xl font-bold hover:bg-slate-800 transition-all flex items-center justify-center space-x-2">
            ...
            <span className="text-xs">Copy Result</span>
          </button>
        </div>
      </div>
    </div>
...
const ManufacturerCell: React.FC<{ manufacturerId: string }> = ({ manufacturerId }) => {
  ...
  if (loading) return <div className="animate-pulse space-y-1"><div className="h-3 bg-slate-100 rounded w-20"></div><div className="h-2.5 bg-slate-50 rounded w-12"></div></div>;
  if (!data) return <span className="text-slate-300">Unspecified</span>;

  return (
    <div className="max-w-[120px] md:max-w-[160px]">
      <div className="text-[10px] md:text-xs font-bold text-slate-800 truncate">{data.name}</div>
      <div className="hidden md:flex items-center space-x-1.5 text-[9px] text-slate-500">
...
const Inventory: React.FC = () => {
  ...
  return (
    <div className="space-y-4 pb-12 animate-in fade-in duration-500">
      ...
      <div className="flex overflow-x-auto no-scrollbar space-x-2 pb-2 -mx-1 px-1">
        {quickLinks.map((link) => (
          <button
            key={link}
            className="whitespace-nowrap px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-600 hover:bg-slate-50 transition-all shadow-sm flex items-center space-x-1.5"
          >
            <div className="w-1 h-1 rounded-full bg-amber-400"></div>
            <span>{link}</span>
          </button>
        ))}
        <button className="whitespace-nowrap px-3 py-1.5 bg-slate-900 rounded-lg text-[10px] font-bold text-white shadow-lg shadow-slate-900/10">
          + New Entry
        </button>
      </div>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-black text-slate-800 tracking-tight">Inventory Control</h1>
            <p className="text-slate-500 font-medium text-xs">Batch lifecycle & shelf-life monitoring</p>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:max-w-2xl">
          <div className="flex items-center space-x-2 bg-white rounded-xl shadow-sm border border-slate-200 px-3 py-2 flex-1 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
            <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input
              type="text"
              placeholder="Search batches..."
              className="flex-1 outline-none text-xs text-slate-700 bg-transparent font-medium min-w-0"
              value={searchTerm}
          ...
          <div className="flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm shrink-0">
            <button
              ...
              className="flex-1 sm:flex-none px-3 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 border-r border-slate-100 transition-colors flex items-center justify-center space-x-2"
            >
              <span>CSV</span>
            </button>
            <button
              ...
              disabled={isExporting}
              className="flex-1 sm:flex-none px-3 py-2 text-[10px] font-bold text-emerald-600 hover:bg-emerald-50 transition-colors flex items-center justify-center space-x-2"
            >
              <span>Excel</span>
            </button>
          </div>
      ...
      {/* Bulk Operations Bar */}
      <div className={`transition-all duration-300 overflow-hidden ${selectedIds.size > 0 ? 'max-h-32 opacity-100 mb-4' : 'max-h-0 opacity-0'}`}>
        <div className="bg-slate-900 border border-slate-800 rounded-xl md:rounded-2xl p-3 md:p-4 flex flex-col md:flex-row items-center justify-between gap-3 shadow-2xl">
          <div className="flex items-center space-x-3 text-white">
            <div className="bg-emerald-500 text-white w-8 h-8 rounded-lg flex items-center justify-center text-sm font-black shadow-lg">
              {selectedIds.size}
            </div>
            <div>
              <span className="font-bold text-xs block">Items Selected</span>
            </div>
          </div>
          <div className="flex items-center space-x-2 w-full md:w-auto">
            <button className="flex-1 md:flex-none px-4 py-2 bg-red-600 text-white rounded-lg text-[10px] font-bold hover:bg-red-500 transition-all">
              Archive
            </button>
            <button className="flex-1 md:flex-none px-4 py-2 bg-slate-800 text-slate-300 rounded-lg text-[10px] font-bold hover:bg-slate-700 transition-all">
              Bulk Released
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {isLoading ? (
          <div className="p-16 text-center">
            ...
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px]">Syncing Repository...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-[900px] text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-4 py-3 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer group" onClick={() => requestSort('name')}>
                  ...
                  <th className="px-4 py-3 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">Manufacturer</th>
                  <th className="px-4 py-3 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer group" onClick={() => requestSort('qty')}>
                    <div className="flex items-center space-x-1">
                      <span>Stock Level</span>
                      <svg className="w-3 h-3 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>
                    </div>
                  </th>
                  <th className="px-4 py-3 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer group" onClick={() => requestSort('expiry')}>
                    <div className="flex items-center space-x-1">
                      <span>Expiry Date</span>
                      <svg className="w-3 h-3 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    </div>
                  </th>
                  <th className="px-4 py-3 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th>
                  <th className="px-4 py-3 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Actions</th>
                </tr>
              </thead>
                ...
                {sortedItems.map((item) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-all cursor-pointer group ${isSelected ? 'bg-emerald-50/50' :
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center space-x-2">
                          <div className={`font-bold text-xs md:text-sm ${isExpired || isCriticalExpiry ? 'text-red-900' : 'text-slate-800'
                            }`}>{item.name}</div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center space-x-2">
                          <div className="flex flex-col">
                            <span className={`text-sm md:text-base font-black ${isLowStock ? 'text-amber-600' : 'text-slate-900'}`}>{item.qty}</span>
                            <span className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase -mt-0.5">{item.unit}</span>
                          </div>
                          <button
                            onClick={(e) => openConverter(item.qty, item.unit, e)}
                            className="p-1 bg-slate-100 hover:bg-emerald-100 text-slate-500 hover:text-emerald-700 rounded-md transition-all shadow-sm"
                          >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col">
                          <span className={`text-xs font-bold ${isExpired || isCriticalExpiry ? 'text-red-600' : 'text-slate-600'}`}>
                            {new Date(item.expiry).toLocaleDateString()}
                          </span>
                          {!isExpired && (isNearExpiry || isCriticalExpiry) && (
                            <span className={`text-[9px] font-black uppercase ${isCriticalExpiry ? 'text-red-400' : 'text-amber-400'}`}>
                              {daysLeft} days left
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider border ${isExpired ? 'bg-red-100 text-red-800 border-red-200' :
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end space-x-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="p-1.5 bg-emerald-600 text-white rounded-md hover:scale-110 transition-all"
                          >
