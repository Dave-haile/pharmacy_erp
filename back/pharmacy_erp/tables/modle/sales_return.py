...
class SalesReturn(models.Model):
    ...
    reference_invoice = models.CharField(max_length=50, blank=True, db_index=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new and not self.posting_number:
            self.posting_number = f"SRET-{self.created_at.year}-{self.pk:04d}"
            super().save(update_fields=["posting_number"])

    def __str__(self):
        return self.posting_number or f"Sales Return #{self.id}"
